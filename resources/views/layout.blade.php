                    ...
                    @foreach($connections as $name => $connection)
                        @if(!empty($connection['host']))
                            <li @if($name == $conn)class="active"@endif>
                                <a href=" {{ admin_route('redis-index', ['conn' => $name]) }}">
                                    <i class="fa fa-database"></i> {{ $name }}
                    ...
                    <table class="table table-striped">
                        @foreach($connections[$conn] as $name => $value)
                            <tr>
                                ...
                                <td>
                                    <span class="label label-primary">{{ is_array($value) ? json_encode($value) : (is_null($value) ? 'null' : $value) }}</span>
                                </td>
                            </tr>
                        @endforeach
                    </table>
                    ...
                    @foreach($info as $part => $detail)
                        <div class="panel box box-default no-border">
                            ...
                            <span class="box-title">
                                <a data-toggle="collapse" data-parent="#accordion" href="#collapse{{ $part }}"
                                   aria-expanded="false" class="collapsed" style="font-size: 14px;">
                                    {{ $part }}
                                </a>
                            </span>
                            </div>
