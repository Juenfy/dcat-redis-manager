                        ...
                        <table class="table table-hover">
                            ...
                            @foreach($data['value'] as $index => $value)
                                <tr>
                                    <td>{{ $index }}</td>
                                    <td>
                                        <a class="list-item" data-type="textarea" data-pk="{{ $index }}" data-url="{{ admin_route('redis-update-key', ['type' => 'list', 'conn' => $conn, 'key' => $data['key']]) }}">{{ $value }}</a></td>
                                    <td>
        ...
        $(function () {
            ...
            $('.list-push').on('click', function (event) {
                ...
                $.ajax({
                    ...
                    data: params,
                    success: function(result) {
                        Dcat.success('Push success.');
//                    $.pjax.reload('#pjax-container');
                        $.pjax({container:'#pjax-container', url: '{{ admin_route('redis-edit-key') }}' + '?conn={{ $conn }}&key='+key });
                    }
                });
