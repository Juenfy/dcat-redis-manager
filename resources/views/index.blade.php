...
<div class="box box-primary">
    ...
    <form class="form-horizontal" action="{{ admin_route('redis-index') }}" pjax-container>
        <div class="box-body">
            ...
            <div class="form-group">
                <div class="col-sm-offset-2 col-sm-9">
                    ...
                    <a class="btn btn-warning btn-sm pull-right"  style="margin-right: 5px;" href="{{ admin_route('redis-console', ['conn' => $conn]) }}"><i class="fa fa-terminal"></i>&nbsp;&nbsp;Console</a>
                    ...
                    <div class="btn-group pull-right btn-group-sm" style="margin-right: 5px;">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li><a href="{{ admin_route('redis-create-key', ['conn' => $conn, 'type' => 'string']) }}">string</a></li>
                            <li><a href="{{ admin_route('redis-create-key', ['conn' => $conn, 'type' => 'list']) }}">list</a></li>
                            <li><a href="{{ admin_route('redis-create-key', ['conn' => $conn, 'type' => 'hash']) }}">hash</a></li>
                            <li><a href="{{ admin_route('redis-create-key', ['conn' => $conn, 'type' => 'set']) }}">set</a></li>
                            <li><a href="{{ admin_route('redis-create-key', ['conn' => $conn, 'type' => 'zset']) }}">zset</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <tr>
                    ...
                    <td>
                        @php($type = $key[1]->getPayload())
                            <span class="label label-{{ \Juenfy\DcatRedisManager\RedisManager::typeColor($type) }}">{{ $type }}</span>
                    </td>
                    <td>{{ $key[2] }}</td>
                    <td>
                        <a href="{{ admin_route('redis-edit-key', ['key' => $key[0], 'conn' => $conn]) }}"><i class="fa fa-edit"></i></a>
                        &nbsp;
...
<script>
    $(function () {
        $('a.key-delete').on('click', function (e) {
            ...
            Dcat.confirm("Delete key ["+key+"] ?", null, function () {
                var params = {
                    conn: "{{ $conn }}",
                    key: key,
                    _token: Dcat.token
                ...
                $.ajax({
                    url: '{{ admin_route('redis-key-delete') }}',
                    type: 'DELETE',
                    data: params,
                    success: function (result) {
                        Dcat.success('Key ' + key + ' deleted');
        ...
        $('.key-delete-multi').click(function () {
            ...
            Dcat.confirm("Delete selected keys ?", null, function () {
                var params = {
                    conn: "{{ $conn }}",
                    key: keys,
                    _token: Dcat.token
                ...
                $.ajax({
                    ...
                    data: params,
                    success: function (result) {
                        Dcat.success(params.key.length + ' keys deleted');
                        $.pjax.reload('#pjax-container');
                    }
                });
            });
